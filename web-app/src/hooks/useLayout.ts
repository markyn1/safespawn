...
export function useLayout(initialFormat: string = "instagram43") {
    ...
    // Layout Profiles (Visuals)
    const [layoutProfileName, setLayoutProfileName] = useState("default");
    const [availableLayoutProfiles, setAvailableLayoutProfiles] = useState<string[]>(["default"]);

    // Social Profiles (Identity / Variables)
    const [socialProfileName, setSocialProfileName] = useState("default");
    ...
    const fetchFormatConfig = async () => {
        try {
            setLoading(true);
            const res = await api.get(`/settings/formats/${formatName}/profiles/${encodeURIComponent(layoutProfileName)}`);
    ...
    const saveConfig = async () => {
        ...
        let profileToSave = layoutProfileName;

        if (layoutProfileName === "default" && !isSuperuser) {
            let newName = "Cópia do padrão";
            let i = 1;
            const existing = availableLayoutProfiles || [];
            ...
            try {
                ...
                setLayoutProfileName(createdName);
                await fetchLayoutProfiles();
            } catch (err: any) {
        ...
        try {
            ...
            toast.success(profileToSave !== layoutProfileName ? `Layout salvo no novo perfil "${profileToSave}".` : `Layout "${layoutProfileName}" salvo!`);
            if (profileToSave !== layoutProfileName) fetchLayoutProfiles();
            return true;
        } catch (error) {
    ...
    const fetchLayoutProfiles = async () => {
        try {
            const res = await api.get(`/settings/formats/${formatName}/profiles`);
            setAvailableLayoutProfiles(res.data.profiles || ["default"]);
        } catch (err) { }
    };
    ...
    const fetchProfileIdentity = async () => {
        try {
            // Find current social profile record to get its ID if needed, 
            // but the route accepts name too now (based on my backend update)
            ...
            if (socialProfileName === "default") {
                setProfileIdentity({
                    display_name: "", username: "", contact: "", genre: "", custom_vars: {}
                });
            }
        }
    };
    ...
    const fetchGlobalVars = async () => {
        try {
            const res = await api.get("/user/variables");
            setGlobalVars(res.data.global_vars || {});
        } catch (err) { }
    };
    ...
    return {
        ...
        layoutProfileName, setLayoutProfileName,
        availableLayoutProfiles, setAvailableLayoutProfiles,
        socialProfileName, setSocialProfileName,
        availableSocialProfiles, setAvailableSocialProfiles,
